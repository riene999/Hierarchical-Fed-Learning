        ...
        while(True):
            for input, target in data_loader:
                ...
                if 'clip' in kwargs.keys() and kwargs['clip'] > 0:
                    torch.nn.utils.clip_grad_norm_(parameters=self.global_model.parameters(), max_norm=kwargs['clip'])
                ...
                if 'clip' in kwargs.keys() and kwargs['clip'] > 0:
                    torch.nn.utils.clip_grad_norm_(parameters=client_model.parameters(), max_norm=kwargs['clip'])
            ...
            if (step_count >= num_steps):
                break
        with torch.no_grad():
            return torch.nn.utils.parameters_to_vector(client_model.parameters())
